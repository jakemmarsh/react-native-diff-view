...
import { insertHunk, textLinesToHunk } from './insert_hunk';
import {
...
type RawCodeOrLines = string | string[];
...
const findCorrespondingValidHunkIndex = (hunks: IHunk[], oldLineNumber: number): number => {
  if (!hunks.length) {
    return -1;
  }
...
const findNearestNormalChangeIndex = ({ changes }: { changes: IChange[] }, start: number): number => {
...
const splitRangeToValidOnes = (hunks: IHunk[], start: number, end: number): number[][] => {
  ...
  if (start < correspondingHunk.oldStart) {
    ...
    return [[start, validEnd], ...splitRangeToValidOnes(hunks, validEnd + 1, end)];
  }
  ...
  return [[validStart, validEnd], ...splitRangeToValidOnes(hunks, validEnd + 1, end)];
};

const expandCodeByValidRange = (hunks: IHunk[], rawCodeOrLines: RawCodeOrLines, [start, end]: number[]): IHunk[] => {
...
export const expandFromRawCode = (
  ...
  return validRanges.reduce(
    (hunks: IHunk[], range: number[]) => expandCodeByValidRange(hunks, rawCodeOrLines, range),
    hunks,
  );
};

export const getCollapsedLinesCountBetween = (previousHunk: IHunk, nextHunk: IHunk): number => {
  if (!previousHunk) {
...
export const expandCollapsedBlockBy = (
  hunks: IHunk[],
  rawCodeOrLines: RawCodeOrLines,
  predicate: (a: number, b: number, c: number) => boolean,
): IHunk[] => {
  const linesOfCode = typeof rawCodeOrLines === 'string' ? rawCodeOrLines.split('\n') : rawCodeOrLines;
  const firstHunk = first(hunks);
  const initialExpandingBlocks = predicate(firstHunk.oldStart - 1, 1, 1) ? [[1, firstHunk.oldStart]] : [];

  const expandingBlocks = hunks.reduce((expandingBlocks, currentHunk, index, hunks) => {
    ...
    const lines = nextHunk ? getCollapsedLinesCountBetween(currentHunk, nextHunk) : linesOfCode.length - oldStart + 1;
